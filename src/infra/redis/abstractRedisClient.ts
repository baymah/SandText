...
export class AsyncRedisClient {
       ...
       * @param key
       * @param value
       * @param timeout timeout in seconds, default to 604800 (1week) 1800(30 minutes)
       */
      public setex(key: string, value: any, timeout = 1800): Promise<any> {
            return new Promise((resolve, reject) => {
                  this.client.set(key, value, (error, reply) => {
                        if (error) {
                              return reject(error)
